...
- **Production** (`NODE_ENV=production`, i.e. `pnpm build && pnpm start`):
  ...
  `env.production.example.txt` and fill in a real connection string, e.g. a
  free MongoDB Atlas cluster).

To seed a production/Mongo database the same way as local dev:

```bash
cp env.production.example.txt .env.production   # fill in DATABASE_URI, PAYLOAD_SECRET, PATIENT_FIELD_ENCRYPTION_KEY
pnpm seed:prod
```
...
straight from the deployed app itself: log in as admin, click **"Seed
data"** in the nav, then **/admin-tools/seed**. Same underlying logic as
`pnpm seed`, just triggered by a button instead of a script, useful once
there's no local terminal in the loop at all. It can't bootstrap the very
first admin from a completely empty database though. That one time still
needs `pnpm seed:prod` run locally.
...
| Doctor   | doctor@securenorth.health       | doctor-demo-pass     | Any record. A doctor needs full patient context |
...
- **Nurse and doctor** get an ordinary patient-record page: name, DOB,
...
3. **Sign out, sign in as the doctor**, open Jane Roe's record. Same
   plain patient view, real plaintext, even though this doctor didn't
...
7. **Switch back to the nurse or doctor tab and reload the record.**
...
padlock, the TLS handshake, and the request payload, genuinely unreadable
on the wire, not simulated.
...
- **Why AES-256-GCM over CBC:** GCM is authenticated encryption. It
  ...
  which is exactly what steps 6 to 7 above demonstrate live. CBC alone
  gives confidentiality but no integrity check, so a flipped bit would
  just decrypt to silently-wrong plaintext instead of raising an error.
